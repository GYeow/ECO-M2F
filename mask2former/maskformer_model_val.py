...
class MaskFormerVal(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        gating_targets = F.one_hot(torch.LongTensor(
            ...
            num_classes=5).to(torch.float).to(self.device) # Cityscapes

        # gating_targets = F.one_hot(torch.LongTensor(
        #     [self.gating_targets[x['file_name'].split('/')[-1].replace('.jpg', '.png')] for x in batched_inputs]), 
        #     num_classes=5).to(torch.float).to(self.device) # COCO
        

        outputs, gates = self.sem_seg_head(features, gating_targets)
    ...
    def prepare_gating_targets(self, files, slop):
        ...
        gating_targets = {k.replace("_gtFine_panoptic.png", ""): np.argmax(np.array(v) - np.arange(1,6)*slop) 
                          for k,v in data.items()}
